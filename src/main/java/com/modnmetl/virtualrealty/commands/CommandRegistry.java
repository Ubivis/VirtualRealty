...
public class CommandRegistry {
	...
	public static void setupPlaceholders() {
		for (SubCommand subCommand : PLOT_SUB_COMMAND_LIST) {
			PLOT_PLACEHOLDERS.put("%" + subCommand.getSubCommandClassName() + "_command%",
					subCommand.getSubCommandName());
		}
		for (SubCommand subCommand : VRPLOT_SUB_COMMAND_LIST) {
			VRPLOT_PLACEHOLDERS.put("%" + subCommand.getSubCommandClassName() + "_command%",
					subCommand.getSubCommandName());
		}
	...
	public static void addSubCommandToRegistry(Class<?> clazz, CommandType commandType) {
		if (clazz.getSimpleName().equalsIgnoreCase("SubCommand"))
			return;
	...
	public static Optional<SubCommand> getSubCommand(String name, CommandType commandType) {
		switch (commandType) {
		case PLOT: {
			return PLOT_SUB_COMMAND_LIST.stream()
					.filter(subCommand -> subCommand.getSubCommandClassName().equalsIgnoreCase(name)
							|| (subCommand.getAlias() != null && subCommand.getAlias().equalsIgnoreCase(name)))
					.findAny();
		}
		case VRPLOT: {
			return VRPLOT_SUB_COMMAND_LIST.stream()
					.filter(subCommand -> subCommand.getSubCommandClassName().equalsIgnoreCase(name)
							|| (subCommand.getAlias() != null && subCommand.getAlias().equalsIgnoreCase(name)))
					.findAny();
		}
	...
	public static String getSubCommandName(String placeholder, CommandType commandType) {
		Optional<SubCommand> command = getSubCommand(placeholder.replaceAll("%", "").replaceAll("_command", ""),
				commandType);

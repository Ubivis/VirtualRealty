...
public class GridStructure {
	...
	public GridStructure(Player viewer, int length, int height, int width, int cuboidId, World world, long displayTicks,
	...
	public static boolean isCuboidGridDisplaying(Player player, int cuboidId) {
		return ACTIVE_GRIDS.containsKey(player.getUniqueId())
				&& ACTIVE_GRIDS.get(player.getUniqueId()).contains(cuboidId);
	}
	...
	public void preview(Location playerPreviewLocation, boolean visualization, boolean colliding) {
		...
		if (plot != null) {
			...
			switch (direction) {
			case SOUTH: {
				bottomLeftCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() + 1,
						previewLocation.getBlockY() - 10, previewLocation.getBlockZ() - 1);
				topRightCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() - getWidth(),
						previewLocation.getBlockY() + getHeight(), previewLocation.getBlockZ() + getLength());
				break;
			}
			case WEST: {
				bottomLeftCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() + 1,
						previewLocation.getBlockY() - 10, previewLocation.getBlockZ() + 1);
				topRightCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() - getLength(),
						previewLocation.getBlockY() + getHeight(), previewLocation.getBlockZ() - getWidth());
				break;
			}
			case NORTH: {
				bottomLeftCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() - 1,
			...
			case EAST: {
				bottomLeftCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() + getLength(),
						previewLocation.getBlockY() - 10, previewLocation.getBlockZ() - 1);
				topRightCorner = new Location(getCreatedWorld(), previewLocation.getBlockX() - 1,
						previewLocation.getBlockY() + getHeight(), previewLocation.getBlockZ() + getWidth());
				break;
			}
			default:
		...
		for (Block borderBlock : new LinkedList<>(borderBlocks)) {
			// top grid floor
			blocks.add(borderBlock.getLocation()
					.add(0, topRightCorner.getBlockY() - borderBlock.getLocation().getBlockY(), 0).getBlock());
			// bottom grid floor
			blocks.add(borderBlock.getLocation()
					.subtract(0, borderBlock.getLocation().getBlockY() - bottomLeftCorner.getBlockY(), 0).getBlock());
		...
		swapBlocks(blocks, visualization && colliding);
		if (displayTicks == 0)
			return;
		new BukkitRunnable() {
			@Override
	...
	private void swapBlocks(LinkedList<Block> blocks, boolean collidingArea) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(VirtualRealty.getInstance(), () -> {
			for (Block block : blocks) {
				...
				convertedBlock = new VirtualBlock(block.getX(), block.getY(), block.getZ(),
				...
				viewer.sendBlockChange(blockLocation, ((plot != null && plot.getID() == cuboidId) ? orangeBlockData
						: collidingArea ? redBlockData : greenBlockData));
